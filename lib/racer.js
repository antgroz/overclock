...
Racer.prototype.get = function (name) {
  return this.tasks[name];
};

Racer.prototype.has = function (name) {
  return !!this.tasks[name] || null;
};

Racer.prototype.del = function (name) {
...
Racer.prototype.start = function (name) {
  if (name) {
    var task = this.tasks[name];
    if (!task) {
...
Racer.prototype.pipe = function (name, event) {
...
Racer.prototype.unpipe = function (name, event) {
  if (event && !~EVENTS.indexOf(event)) {
    throw new Error('Event is invalid');
  }
