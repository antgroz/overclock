...
import { createTask, Task, TaskListener, TaskOptions } from './tasks';
 ...
 */
export class Principe extends EventEmitter {
  private readonly _pipes = new Map<typeof EVENTS[number], TaskListener>();
  private readonly _tasks = new Map<string, Task>();

  /**
   * @param options - Principe options
   */
  public constructor(options: PrincipeOptions) {
    super();
   ...
   * @returns - This
   */
  public add(options: TaskOptions) {
   ...
   * @returns - This
   */
  public pipe(task: string, event?: typeof EVENTS[number]): this {
   ...
   * @returns - This
   */
  public unpipe(task: string, event?: typeof EVENTS[number]): this {
    if (event && !EVENTS.includes(event)) {
      const message = `Event '${event}' is invalid`;
      throw new ParameterError(message);
    }
