...
class Base extends EventEmitter {
  ...
  constructor(options) {
    ...
    for (const [symbol, name] of EVENT_MAP) {
      this.on(symbol, (...args) => this.emit(name, ...args));
    }
    ...
    this._initialCapacity = initialCapacity ?? this._factoryCapacity;
  ...
  _starting() {
    ...
    const data = { task: this, error: null, result: null, details: null };
    this.emit(kStarting, data);
  }
  ...
  _spawning() {
    const data = { task: this, error: null, result: null, details: null };
    this.emit(kSpawning, data);
  }

  /**
   * Signal having spawned
   *
   * @param {object} [options] - Signal options
   * @param {Error | null} [options.error=null] - Error
   * @param {*} [options.result=null] - Result
   * @protected
   */
  _spawned(options = { error: null, result: null }) {
    const { error, result } = options;
    const data = { task: this, error, result, details: null };
   ...
   * @protected
   */
  _stopping(options = { error: null }) {
    this._isStopping = true;
    this._stoppingAt = new Date();

    const { error } = options;
    const data = { task: this, error, result: null, details: null };
    this.emit(kStopping, data);
  }
  ...
  _spawn() {
    ...
    if (generationLimit === 0) {
      ...
      error = new OCK_ERR_TSK_LC_STOP_GEN_LIM_RCHD(name, generationLimit);
      doStop = true;
  ...
  async _tryStop() {
    ...
    const successPromise = new Promise((resolve) => {
      const listener = () => {
        ...
        this.removeListener(kTock, listener);
        this._stopped();
        resolve();
      };

      this.on(kTock, listener);
    });
